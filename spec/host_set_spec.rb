...
describe HPE3PARClient::HostSetManager do
  before(:all) do
  ...
  it 'validate create hostset' do
    ...
    client = HPE3PARClient::HostSetManager.new(http)
  ...
  it 'validate find all hostsets of a host with hostset filter supported as true' do
    ...
    client = HPE3PARClient::HostSetManager.new(http, true)
    client.instance_variable_set('@http', http)
  ...
  it 'validate find all hostsets with hostset filter supported as false' do
    ...
    members.each do |host_set|
      host_sets.push(HPE3PARClient::HostSet.new(host_set))
    end
  ...
  it 'validate get hostset by name which is empty or has just a space' do
    ...
    http = HPE3PARClient::HTTPJSONRestClient.new(@url, false, false, false, nil)
    client = HPE3PARClient::HostSetManager.new(http)
    expect { client.get_host_set(host_set_name) }.to raise_error(HPE3PARClient::HPE3PARException)
  end
  ...
  it 'validate get hostset by name' do
    ...
    client = HPE3PARClient::HostSetManager.new(http)
    client.instance_variable_set('@http', http)
    expect(class_object_to_hash(client.get_host_set(host_set_name))).to eq(class_object_to_hash(HPE3PARClient::HostSet.new(single_host_set_response_body)))
  end
  ...
  it 'validate add host to hostset' do
    host_set_name = 'sample-host-set1'
    host_name = ['sample-host']
    mod_request = { 'action' => HPE3PARClient::SetCustomAction::MEM_ADD,
  ...
  it 'validate remove host to hostset' do
    host_set_name = 'sample-host-set1'
    host_name = ['sample-host']
    mod_request = { 'action' => HPE3PARClient::SetCustomAction::MEM_REMOVE, 'setmembers' => host_name }
    http = HPE3PARClient::HTTPJSONRestClient.new(@url, false, false, false, nil)
    expected_response = [nil, nil]
    allow(http).to receive(:put).with("/hostsets/#{host_set_name}", body: mod_request).and_return(expected_response)
    client = HPE3PARClient::HostSetManager.new(http)
