...
describe HPE3PARClient::FlashCacheManager do
  before(:all) do
  ...
  it 'validate get flash cache' do
    ...
    client = HPE3PARClient::FlashCacheManager.new(http)
    client.instance_variable_set('@http', http)
    expect(class_object_to_hash(client.get_flash_cache)).to eq(class_object_to_hash(HPE3PARClient::FlashCache.new(flash_cache_output)))
  end
  ...
  it 'validate create flash cache with miniumum paramtere i.e. sizeinGib' do
    ...
    response = { 'date' => ['Fri, 04 Aug 2017 10:23:53 GMT'], 'server' =>
        ['hp3par-wsapi'], 'cache-control' => ['no-cache'],
                 ...
                 'connection' => ['close'] }
    body = { 'links' => [{ 'href' => 'https://15.212.192.252:8080/api/v1/flashcache',
