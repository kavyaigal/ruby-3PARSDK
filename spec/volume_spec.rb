...
describe HPE3PARClient::VolumeManager do
  before(:all) do
  ...
  it 'validate get all volumes' do
    ...
    ci = HPE3PARClient::VolumeManager.new(http, ssh, app_type)
    ci.instance_variable_set('@http', http)
    expect(ci.get_volumes(HPE3PARClient::VolumeCopyType::BASE_VOLUME).length).to eq(response[1]['members'].length)
    expect(ci.get_volumes(HPE3PARClient::VolumeCopyType::BASE_VOLUME)[0].name).to eq(response[1]['members'][0]['name'])
  end
  ...
  it 'validate get all snapshots' do
    ...
    ci = HPE3PARClient::VolumeManager.new(http, ssh, app_type)
    ci.instance_variable_set('@http', http)
    expect(ci.get_volumes(HPE3PARClient::VolumeCopyType::VIRTUAL_COPY).length).to eq(data_hash['members'].length)
    expect(ci.get_volumes(HPE3PARClient::VolumeCopyType::VIRTUAL_COPY)[0].size_mib).to eq(data_hash['members'][0]['sizeMiB'])
  end
  ...
  it 'validate get_online_physical_copy_status' do
    ...
    volume = HPE3PARClient::VolumeManager.new(http, ssh, app_type)
    ...
    expect(volume.get_online_physical_copy_status(volume_name)).to eq(HPE3PARClient::TaskStatus::DONE)
  end
