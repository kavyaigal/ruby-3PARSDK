...
describe HPE3PARClient::HostManager do
  before(:all) do
  ...
  it 'validate get all hosts' do
    ...
    client = HPE3PARClient::HostManager.new(http)
  ...
  it 'validate get single host by name' do
    ...
    client = HPE3PARClient::HostManager.new(http)
    client.instance_variable_set('@http', http)
    expect(client.get_host(host_name).name).to eq(HPE3PARClient::Host.new(single_host_response_body).name)
    expect(class_object_to_hash(client.get_host(host_name).fcpaths)).to eq(class_object_to_hash(HPE3PARClient::Host.new(single_host_response_body).fcpaths))
  end
  ...
  it 'validate get host by name which is empty or has just a space' do
    ...
    http = HPE3PARClient::HTTPJSONRestClient.new(@url, false, false, false, nil)
    client = HPE3PARClient::HostManager.new(http)
    expect { client.get_host(host_name) }.to raise_error(HPE3PARClient::HPE3PARException)
  end
  ...
  it 'validate querying a host from FC WWN host' do
    ...
    client = HPE3PARClient::HostManager.new(http)
    client.instance_variable_set('@http', http)
    expect(client.query_host_by_fc_path(wwn).name).to eq(HPE3PARClient::Host.new(query_host_body['members'][0]).name)
  end
  ...
  it 'validate querying a host from an iSCSI initiator host' do
    ...
    client = HPE3PARClient::HostManager.new(http)
    client.instance_variable_set('@http', http)
    expect(client.query_host_by_iscsi_path(iqn).name).to eq(HPE3PARClient::Host.new(query_host_body['members'][0]).name)
  end
  ...
  it 'validate get all of the VLUNs on a specific host when vlun_query_supported is set to true' do
    ...
    host_obj = HPE3PARClient::HostManager.new(http, true)
  ...
  it 'validate get all of the VLUNs on a specific host when vlun_query_supported is set to false' do
    ...
    host_obj.instance_variable_set('@http', http)
    vlun_obj = HPE3PARClient::VlunManager.new(http)
