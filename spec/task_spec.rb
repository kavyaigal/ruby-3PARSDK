...
describe HPE3PARClient::TaskManager do
  before(:all) do
  ...
  it 'validate get all tasks' do
    ...
    task = HPE3PARClient::TaskManager.new(@url)
  ...
  it 'validate get task by id' do
    ...
    task = HPE3PARClient::TaskManager.new(@url)
    task.instance_variable_set('@http', http)
    expect(task.get_task(task_id).name).to eq(HPE3PARClient::Task.new(all_tasks['members'][0]).name)
    expect(task.get_task(task_id).type).to eq(HPE3PARClient::Task.new(all_tasks['members'][0]).type)
  end
