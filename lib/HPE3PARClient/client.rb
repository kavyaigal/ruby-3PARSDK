...
module HPE3PARClient
  class Client
    def initialize(api_url, debug: false, secure: false, timeout: nil,
                   suppress_ssl_warnings: false, app_type: 'ruby-3parclient',
                   log_file_path: nil)

      unless api_url.is_a?(String)
        raise HPE3PARClient::HPE3PARException.new(nil,
    ...
    private def init_log
      ...
      HPE3PARClient.logger = MultiLog.new(:level => @log_level, :loggers => client_logger)
    end
    ...
    private def check_WSAPI_version
      begin
        ...
        if ex_message && ex_message.include?('SSL Certificate Verification Failed')
          raise HPE3PARClient::SSLCertFailed
        else
          ...
          'not supported."
          raise HPE3PARClient::HPE3PARException(message: msg)
        end
      end
    ...
    private def set_ssh_options(username, password, port=22, conn_timeout=nil)
      @ssh=HPE3PARClient::SSH.new(@api_url.split("//")[1].split(":")[0], username, password)
    end
    ...
    private def compare_version(api_version)
      ...
      if @current_version < @min_version
        ...
        raise HPE3PARClient::UnsupportedVersion.new(nil, err_msg)
      end
    ...
    # * HPE3PARClient::HTTPConflict
    #       - EXISTENT_FLASH_CACHE - The Flash Cache already exists.
    # * HPE3PARClient::HTTPForbidden
    ...
    #     type qos_rules: Hash
    #     qos_rules = {
    #         'priority'=> 2,         # Refer HPE3PARClient::QoSpriorityEnumeration for complete enumeration 
    ...
    # * target_name: Name of the target object on which the QoS rule will be created.
    #     type target_name: String
    # * target_type: Type of QoS target, either vvset or sys.Refer HPE3PARClient::QoStargetTypeConstants for complete enumeration
    #     type target_type: String
    # * qos_rules: QoS options
    #     type qos_rules: Hash
    #     qos_rules = {
    ...
    # * HPE3PARClient::HTTPInternalServerError
    ...
    #      type optional: Hash
    #        optional = {
    #            'persona'=> 1,                  # Refer HPE3PARClient::HostPersona for complete enumeration.
    ...
    #            'pathOperation'=> 1,             # Refer HPE3PARClient::HostEditOperation for complete enumeration
    ...
    #            'chapOperation'=> 1,             # Refer HPE3PARClient::HostEditOperation for complete enumeration
    #            'chapOperationMode'=> TARGET,    # Refer HPE3PARClient::ChapOperationMode for complete enumeration
    ...
    #      type action: Refer values of HPE3PARClient::SetCustomAction::MEM_ADD and HPE3PARClient::SetCustomAction::MEM_REMOVE
    ...
    # ==== Raises
    #
    # * HPE3PARClient::HPE3PARException
    ...
    #                                         # apply to online copy - HPE3PARClient::TaskPriority)
    #      }
    ...
    #                                       # tuned.
    #       'conversionOperation' => 1,     # conversion operation enum. Refer HPE3PARClient::VolumeConversionOperation
    ...
    #      type name: HPE3PARClient::SetCustomAction
    ...
    # * HPE3PARClient::HTTPServiceUnavailable  
    ...
    #      type name: Integer. Refer HPE3PARClient::PortLinkState for complete enumeration.
    #
    # ==== Returns
    #
    ...
    def logout
      unless @log_file_path.nil?
        if HPE3PARClient.logger != nil
          HPE3PARClient.logger.close
          HPE3PARClient.logger = nil
        end
      end
      begin
        @http.unauthenticate
