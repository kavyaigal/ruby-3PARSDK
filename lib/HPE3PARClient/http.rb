  ...
  class HTTPJSONRestClient
    ...
    def get(url, **kwargs)
      ...
      response = HTTParty.get(api_url + url,
                              headers: headers,
                              verify: secure, logger: HPE3PARClient.logger,
    ...
    def process_response(response)
      ...
      if response.code != 200
        if !body.nil? && body.key?('code') && body.key?('desc')
          exception = HPE3PARClient.exception_from_response(response, body)
          begin
            raise exception
          rescue => ex
            log_exception(ex, caller_locations(1, 1)[0].label)
            raise ex
          end
        end
      end
    ...
    def log_exception(exception, caller_location)
      ...
      HPE3PARClient.logger.error(err_msg)
    end
