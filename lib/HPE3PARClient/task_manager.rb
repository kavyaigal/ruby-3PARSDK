  ...
  class TaskManager
    ...
    def get_task(task_id)
      if task_id.is_a? Integer
        ...
        raise HPE3PARClient::HTTPBadRequest.new(
    ...
    def cancel_task(task_id)
      if task_id.is_a? Integer
        _body = Hash.new
        _body['action'] = HPE3PARClient::TaskAction::CANCEL_TASK
    ...
    def _wait_for_task_to_end_loop(task_id, poll_rate_secs)
      ...
      while task != nil do #loop begin
        state = task.status
        if state == HPE3PARClient::TaskStatus::DONE
          break
        end

        if state == HPE3PARClient::TaskStatus::CANCELLED
          HPE3PARClient.logger.info("Task #{task.task_id} was CANCELLED!!!")
          break
        end

        if state == HPE3PARClient::TaskStatus::FAILED
          msg = "Task '#{task.task_id}' has FAILED!!!"
          HPE3PARClient.logger.info(msg)
          raise HPE3PARClient::HPE3PARException.new(message: msg)
        end

        if state == HPE3PARClient::TaskStatus::ACTIVE
          sleep(poll_rate_secs)
          task = get_task(task.task_id);
          HPE3PARClient.logger
              .info("Polling task #{task.task_id} current status: #{HPE3PARClient::TaskStatus.get_string(task.status)}")
        end
